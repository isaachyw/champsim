...
std::string O3_CPU::find_trace_short_name(std::string &full_path,
                                          O3_CPU::NameKind name_kind) {
  fs::path p = full_path;
...
void O3_CPU::initialize_core() {
  ...
  // twig_prefetch_match = twig_prefetcher.init(short_name,
  // use_twig_prefetcher);
}
...
uint32_t O3_CPU::init_instruction(ooo_model_instr arch_instr) {
  ...
  if (!pt) {
    ...
    // usually be determined immediately after the instruction is decoded
    // without waiting for the stack pointer's dependency chain to be resolved.
    // We're doing it here because we already have writes_sp and reads_other
    // handy, and in ChampSim it doesn't matter where before execution you do
    // it.
    if (writes_sp) {
  ...
  if (arch_instr.is_branch) {
    ...
    std::pair<uint64_t, uint8_t> btb_result =
        btb_prediction(arch_instr.ip, arch_instr.branch_type, &predict_latency);
    ...
    uint8_t branch_prediction =
        predict_branch(arch_instr.ip, predicted_branch_target, always_taken,
    ...
    l1i_prefetcher_branch_operate(
        arch_instr.ip, arch_instr.branch_type, predicted_branch_target,
        branch_prediction != 0, always_taken != 0, arch_instr.branch_target);
    ...
    update_btb(arch_instr.ip, arch_instr.branch_target, arch_instr.branch_taken,
               arch_instr.branch_type);
    last_branch_result(arch_instr.ip, arch_instr.branch_target,
                       arch_instr.branch_taken, arch_instr.branch_type);

    //        assert((arch_instr.branch_target != 0 && arch_instr.branch_taken
    //        == 1) ||
    //               (arch_instr.branch_target == 0 && arch_instr.branch_taken
    //               == 0));
    // twig_record.lookup(
    //         arch_instr.instr_id,
    //         arch_instr.ip,
    ...
    // twig_prefetcher.prefetch(arch_instr.ip, arch_instr.branch_target,
    // arch_instr.branch_type, this);
  }
...
uint32_t O3_CPU::check_rob(uint64_t instr_id) {
  ...
  for (uint32_t i = ROB.head, count = 0; count < ROB.occupancy;
       i = (i + 1 == ROB.SIZE) ? 0 : i + 1, count++) {
...
void O3_CPU::fetch_instruction() {
  ...
  for (auto it = IFETCH_BUFFER.begin(); it != end; ++it) {
    ...
    dib_t::value_type &dib_set =
        DIB[(it->ip >> LOG2_DIB_WINDOW_SIZE) % DIB_SET];
    auto to_find = it->ip;
    auto way =
        std::find_if(dib_set.begin(), dib_set.end(), [to_find](dib_entry_t x) {
          return x.valid && ((x.addr >> LOG2_DIB_WINDOW_SIZE) ==
                             (to_find >> LOG2_DIB_WINDOW_SIZE));
        });
    if (way != dib_set.end()) {
  ...
  // TODO: Also need to search PREFETCH_BUFFER here. If hit on the oldest block,
  // move it to CACHE. scan through IFETCH_BUFFER to find instructions that need
  // to be translated
  auto itlb_req_begin =
...
void O3_CPU::decode_instruction() {
  ...
  while (available_decode_bandwidth > 0 && DECODE_BUFFER.has_ready() &&
    ...
    dib_t::value_type &dib_set =
        DIB[(db_entry.ip >> LOG2_DIB_WINDOW_SIZE) % DIB_SET];
    auto way =
        std::find_if(dib_set.begin(), dib_set.end(), [db_entry](dib_entry_t x) {
          return x.valid && ((x.addr >> LOG2_DIB_WINDOW_SIZE) ==
                             (db_entry.ip >> LOG2_DIB_WINDOW_SIZE));
        });
  ...
  if (ROB.entry[rob_index].is_memory == 0) {
    if ((ROB.entry[rob_index].executed == INFLIGHT) &&
      ...
      if (ROB.entry[rob_index].branch_mispredicted) {
        auto id = ROB.entry[rob_index].instr_id;
        fetch_resume_cycle =
            current_core_cycle[cpu] + BRANCH_MISPREDICT_PENALTY;
...
void O3_CPU::handle_memory_return() {
    ...
    while (!itlb_entry.instr_depend_on_me.empty()) {
      ...
      if (available_fetch_bandwidth > 0) {
        if ((it->ip >> LOG2_PAGE_SIZE) == (itlb_entry.address) &&
          ...
          it->instruction_pa = (itlb_entry.data << LOG2_PAGE_SIZE) |
                               (it->ip & ((1 << LOG2_PAGE_SIZE) - 1));
        ...
        // not enough fetch bandwidth to translate this instruction this time,
        // so try again next cycle
        break;
      }
    }
  ...
  while (available_fetch_bandwidth > 0 && to_read > 0 &&
    ...
    while (!l1i_entry.instr_depend_on_me.empty()) {
      ...
      if (available_fetch_bandwidth > 0) {
        if ((it->instruction_pa >> LOG2_BLOCK_SIZE) == (l1i_entry.address) &&
        ...
        // not enough fetch bandwidth to mark instructions from this block this
        // time, so try again next cycle
        break;
      }
    }
  ...
  while (to_read > 0 && !DTLB_bus.PROCESSED.empty() &&
    ...
    for (auto sq_merged : dtlb_entry.sq_index_depend_on_me) {
      SQ.entry[sq_merged].physical_address =
    ...
    for (auto lq_merged : dtlb_entry.lq_index_depend_on_me) {
      LQ.entry[lq_merged].physical_address =
          (dtlb_entry.data << LOG2_PAGE_SIZE) |
...
void O3_CPU::retire_rob() {
  if ((ROB.entry[ROB.head].executed != COMPLETED) ||
      (ROB.entry[ROB.head].event_cycle > current_core_cycle[cpu])) {
    return;
  }
