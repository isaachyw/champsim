...
void print_branch_stats() {
    for (uint32_t i = 0; i < NUM_CPUS; i++) {
        ...
        */
        ...
        cout << "Branch Predictor Stats" << endl;
        cout <<"1 "<<ooo_cpu[i].branch_mispredictions/ooo_cpu[i].num_branch<<" "<<ooo_cpu[i].branch_mispredictions<<" \n";
        cout <<ooo_cpu[i].stall_cycle<<endl;
//        for (auto it: ooo_cpu[i].branch_record) {
//            if (!it.second.second.empty()) {
//                cout << it.first << " " << (double) it.second.second.size() / (double) it.second.first << " "
...
void print_dram_stats() {
    ...
    for (uint32_t i = 0; i < DRAM_CHANNELS; i++) {
        cout << " CHANNEL " << i << endl;
        cout << " RQ ROW_BUFFER_HIT: " << setw(10) << DRAM.RQ[i].ROW_BUFFER_HIT << "  ROW_BUFFER_MISS: " << setw(10)
             << DRAM.RQ[i].ROW_BUFFER_MISS << endl;
        cout << " DBUS_CONGESTED: " << setw(10) << DRAM.dbus_congested[NUM_TYPES][NUM_TYPES] << endl;
        cout << " WQ ROW_BUFFER_HIT: " << setw(10) << DRAM.WQ[i].ROW_BUFFER_HIT << "  ROW_BUFFER_MISS: " << setw(10)
...
void print_deadlock(uint32_t i) {
    ...
    for (PACKET &entry: ooo_cpu[i].L1D.MSHR) {
        ...
        std::cout << " address: " << std::hex << entry.address << " full_addr: " << entry.full_addr << std::dec
                  << " type: " << +entry.type;
        std::cout << " fill_level: " << entry.fill_level << " lq_index: " << entry.lq_index << " sq_index: "
                  << entry.sq_index << " event_cycle: " << entry.event_cycle << std::endl;
        ++j;
    }
...
//                break;
    ...
    if(run_to_end)cout<<"run to end"<<endl;
...
//           DRAM_SIZE, DRAM_CHANNELS, 8 * DRAM_CHANNEL_WIDTH, DRAM_MTPS);
    ...
    for (int i = 0; i < NUM_CPUS; i++) {
        ...
        LLC.update_replacement_state = std::bind(&CACHE::llc_update_replacement_state, &LLC, _1, _2, _3, _4, _5, _6, _7,
                                                 _8);
    ...
    while (run_simulation) {
        ...
        for (int i = 0; i < NUM_CPUS; i++) {
            ...
            if(!run_to_end)
            {
                finish= (ooo_cpu[i].num_retired >= (ooo_cpu[i].begin_sim_instr + ooo_cpu[i].simulation_instructions));
            }
            else
            {
                finish = traces[i]->isFinished();
            }
