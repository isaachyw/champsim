...
def single_train_generalize(pt_ignore_outlier: bool):
    baseline_df = parse_all_app(
        Path(
            "/mnt/storage/isaachyw/champsim_pt/input_generalization/simulation_output/ChampSim_fdip_opt_generate"
        ),
        parse_first_pass_one_app,
    )
    hwc_df = parse_all_app(
        Path(
            "/mnt/storage/isaachyw/champsim_pt/input_generalization/simulation_output/ChampSim_fdip_hot_warm_cold_80_50_f_keep_curr_hotter_lru"
        ),
        parse_second_pass_one_app,
    )
    opt_df = parse_all_app(
        Path(
            "/mnt/storage/isaachyw/champsim_pt/input_generalization/simulation_output/ChampSim_fdip_opt"
        ),
        parse_first_pass_one_app,
    )
    ...
    plt.savefig(
        "/mnt/storage/isaachyw/champsim_pt/input_generalization/plots/hwc_speedup%s.pdf"
        % ignore_outlier_str
    )
...
def three_best_generalize(no_zero: bool, prefix: str, local: bool, app_list: list):
    large_dir = Path(
        "/mnt/storage/isaachyw/champsim_pt/input_generalization/simulation_output"
    )
    ...
    if not local:
        ...
        df.to_csv(
            "/mnt/storage/isaachyw/champsim_pt/input_generalization/plots/%s_three_best%s.csv"
            % (prefix, no_zero_str)
        )
        ...
        print(per_df)
        per_df.to_csv(
            "/mnt/storage/isaachyw/champsim_pt/input_generalization/plots/%s_opt_percentage_three_best%s.csv"
            % (prefix, no_zero_str)
        )
        ...
        plot_functions.plot_bar_chart(
            output_path=Path(
                "/mnt/storage/isaachyw/champsim_pt/input_generalization/plots/%s_three_best%s.pdf"
                % (prefix, no_zero_str)
            ),
            df=df,
        ...
        output_path = Path(
            "/mnt/storage/isaachyw/champsim_pt/input_generalization/plots/%s_opt_percentage_three_best%s.pdf"
            % (prefix, no_zero_str)
        )
    else:
        no_zero_str = "_no_zero" if no_zero else ""
        # per_df = pd.read_csv("/Users/isaachywong/Desktop/plots/paper_results/plots/percentage_three_best.csv",
        #                      header=0, index_col=0)
        per_df = pd.read_csv("/mnt/storage/isaachyw/champsim_pt/input_generalization/plots/percentage_three_best.csv",
                             header=0, index_col=0)
        # output_path = Path(
        #     "/Users/isaachywong/Desktop/plots/paper_results/plots/%s_opt_percentage_three_best%s.pdf"
        #     % (prefix, no_zero_str)
        # )
        output_path = Path(
            "/mnt/storage/isaachyw/champsim_pt/input_generalization/plots/%s_opt_percentage_three_best%s.pdf"
