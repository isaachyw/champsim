...
def one_file_cdf(input_path: Path):
    df = pd.read_fwf(input_path, header=0, infer_nrows=100000)
    df = df.apply(cal_hit_access, axis=1)
    df = df.sort_values("HitAccess", ascending=False)
    df["cum_sum"] = df["Count"].cumsum()
    df["CDF"] = round(100 * df.cum_sum / df["Count"].sum(), 2)
    unique_counts = map(lambda x: x / len(df), range(len(df)))
    df["Percentage count"] = unique_counts
    df = df.set_index("Percentage count")
    df.plot(y=["HitAccess"], kind="line")
    plt.savefig(Path("/mnt/storage/isaachyw/champsim_pt/paper_results/try.png"))


def main():
    old_plots()
    # one_file_cdf(Path("/mnt/storage/isaachyw/champsim_pt/opt_access_record/way4/verilator.csv"))
