...
async def run_stack_simulator(trace_dir: Path, output_dir: Path):
    for trace in pt_traces:
        ...
        command_str = (
            "zcat "
            + str(trace_path)
            + "|/mnt/storage/isaachyw/icache_profiling/third_party/stack-distance/stack-distance"
        )
...
async def main():
    output_dir = Path(
        "/mnt/storage/isaachyw/champsim_pt/pt_instruction_footprint_raw_data"
    )
    output_dir.mkdir(exist_ok=True)
    await run_stack_simulator(
        Path("/mnt/storage/isaachyw/champsim_pt/pt_traces"), output_dir
    )
