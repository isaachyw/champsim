...
def all_corr(input_dir: Path):
    output_path = Path("/mnt/storage/isaachyw/champsim_pt/all_correlation.csv")
    with output_path.open(mode="w") as output_file:
        collect_data = {
            "hit / miss": [],
            "miss with insertion / all miss": [],
...
def main():
    ...
    output_dir = Path(
        "/mnt/storage/isaachyw/champsim_pt/opt_access_record_hwc_misclassify"
    )
    ...
    # output_dir = Path("/mnt/storage/isaachyw/champsim_pt/opt_hit_access_variance_hist")
    # output_dir.mkdir(exist_ok=True)
    # for input_path in sorted(input_dir.iterdir()):
    ...
    # all_corr(input_dir)
    all_cdf(input_dir)
    # output_dir = Path("/mnt/storage/isaachyw/champsim_pt/opt_access_record_hit_access_dynamic_hist")
    # output_dir.mkdir(exist_ok=True)
    # for input_path in sorted(input_dir.iterdir()):
