...
#define BASIC_BTB_RAS_SIZE                                                     \
  32 // TODO: Different from the original value 64, need to rerun tests!
...
#define BTB_HOT_LOWER_BOUND 0.8
#define BTB_COLD_UPPER_BOUND 0.45
#define BTB_WARM_SPLIT 0
#define BTB_IGNORE_ALL_HOT false

#define BTB_SORT_HIT_ACCESS false
#define BTB_USE_LRU true
#define BTB_CONSIDER_WARM_TAKEN false
#define BTB_CONSIDER_HOT_TAKEN false
#define BTB_CONSIDER_KEEP true
#define BTB_CONSIDER_TAKEN_HISTORY false

#define BTB_CURR_HOTTER true

// whether use approx

#define BTB_OPT_APPROX true
...
enum class OptAccessRecordType { LONG, SHORT };
...
struct BASIC_BTB_ENTRY {
  ...
  int get_loop_judge() {
    // Return -1 if the last access is taken. Else, return num of takens before
    // the last not taken
    if (taken_history.back())
      return -1;
    int taken_count = 0;
    for (uint64_t i = 1; i < taken_history.size(); i++) {
      if (taken_history[taken_history.size() - 1 - i])
        return taken_count;
      taken_count++;
    }
    return taken_count;
...
enum class CacheType { HOT, WARM, COLD };
...
public:
  ...
  //        vector<uint8_t> candidate_type; // 0-3 is for old branches in btb,
  //        and 4 is for curr branch vector<bool> candidate_taken; uint32_t
  //        hwc_choice; // 0-4: index of victim chosen by hwc uint32_t
  //        opt_choice; // 0-4: index of victim chosen by opt
  //
  //        static uint32_t judge_index(vector<uint64_t> &candidate_ip, uint64_t
  //        target_ip) {
  ...
  //                          uint64_t curr_ip, uint64_t hwc_ip, uint64_t
  //                          opt_ip, HotWarmCold *hwc, O3_CPU *ooo_cpu) {
  ...
  //                candidate_taken.push_back(ooo_cpu->predict_branch(a, 0, 0,
  //                0));
  //            }
  //        }
  //    };
  ...
  HotWarmCold(uint64_t sets, uint64_t ways, double hot_lower, double cold_upper,
              double warm_split)
      : total_sets(sets), total_ways(ways), hot_lower(hot_lower),
  ...
  void init(uint64_t sets, uint64_t ways) {
    ...
    total_ways = ways;
    btb.resize(NUM_CPUS,
               vector<unordered_map<uint64_t, BASIC_BTB_ENTRY>>(sets));
    //        basic_opt.init(sets, ways);
    //        basic_opt.get_btb_pointer(&btb);
  }

  uint64_t num_category() { return category_boundary.size() + 1; }
  ...
  uint64_t find_lru_min(uint64_t set, uint64_t cpu,
                        vector<uint64_t> &candidates) {
    // candidates may contain the current ip, which is not in btb.
    // Use random number to determine whether choosing the current ip, if not,
    // then use lru.
    ...
    if (access_record_type == OptAccessRecordType::LONG) {
      ...
      while (line_stream >> trash) {
        ...
        if (tmp == (uint64_t)RecordType::HIT)
          hit++;
        else if (tmp == (uint64_t)RecordType::MISS_ONLY ||
                 tmp == (uint64_t)RecordType::MISS_INSERT)
          miss++;
      }
  ...
  void init_record(string &trace_name, bool with_twig = false) {
    ...
    auto short_name =
        O3_CPU::find_trace_short_name(trace_name, O3_CPU::NameKind::TRAIN);
    if (with_twig) {
      short_name = "twig_" + short_name;
    }
    fs::path opt_access_record_path =
        with_twig
            ? "/mnt/storage/isaachyw/champsim_pt/opt_access_record_predecoder"
            : "/mnt/storage/isaachyw/champsim_pt/opt_access_record";
    if (opt_approx) {
      opt_access_record_path =
          "/mnt/storage/isaachyw/champsim_pt/approx_opt_access_record";
    }
  ...
  CacheType judge_hot_warm_cold(uint64_t ip) {
    ...
    auto it = branch_record.find(ip);
    assert(it != branch_record.end());
    if (it->second >= hot_lower) // TODO: Change here to > to make it the same
                                 // as my modified code.
  ...
  uint64_t get_set_index(uint64_t ip) { return ((ip >> 2) % total_sets); }
  ...
  uint64_t sort_choose_victim(uint64_t ip, uint64_t cpu, uint64_t set,
                              O3_CPU *ooo_cpu) {
  ...
  bool add_to_candidate(uint64_t ip, vector<vector<uint64_t>> &candidate,
                        bool curr_ip = false) {
    auto hit_access = get_hit_access_ratio(ip);
    if (hit_access < 0)
  ...
  void add_to_candidate(uint64_t ip, O3_CPU *ooo_cpu,
    ...
    if (cache_type == CacheType::COLD) {
      ...
      // TODO: For tage-sc-l the last three parameters is useless, if using
      // other predictor, modify here.
  ...
  uint64_t choose_taken_history(vector<uint64_t> &candidate, uint64_t cpu,
                                uint64_t set) {
    // Return 0 if no victim chosen by this method.
    if (!consider_taken_history)
      return 0;
    int min_length = 8;
  ...
  uint64_t choose_victim_one_category(uint64_t cpu, uint64_t set,
                                      vector<uint64_t> &one_category,
                                      O3_CPU *ooo_cpu) {
    // TODO: Add a parameter judge_taken here to consider whether considering
    // predicted taken or not. If ooo_cpu != nullptr, predict whether taken or
    // not and then determine the victim. If ooo_cpu == nullptr, then we don't
    // consider predict taken here.
    ...
    auto &final_candidates =
        not_taken_candidates.empty() ? one_category : not_taken_candidates;

    if (use_lru) {
      return find_lru_min(set, cpu, final_candidates);
    }
    auto taken_history_choice =
        choose_taken_history(final_candidates, cpu, set);
    if (taken_history_choice != 0)
      return taken_history_choice;
  ...
  bool judge_consider_taken(uint64_t category_index) {
    if (consider_hot_taken && category_index == num_category() - 1)
      return true;
    if (consider_warm_taken && category_index > 0 &&
        category_index < num_category() - 1)
      return true;
    return false;
  }

  uint64_t choose_victim_general(uint64_t ip, uint64_t cpu, uint64_t set,
                                 O3_CPU *ooo_cpu) {
    ...
    for (uint64_t i = 0; i < candidate.size(); i++) {
      if (!candidate[i].empty()) {
        for (auto dont_care_ip : dont_care) {
          assert(std::find(candidate[i].begin(), candidate[i].end(),
                           dont_care_ip) == candidate[i].end());
  ...
  uint64_t choose_victim(uint64_t ip, uint64_t cpu, uint64_t set,
                         O3_CPU *ooo_cpu) {
    vector<uint64_t> cold_candidate;
    vector<uint64_t> warm_not_taken_candidate;
    vector<uint64_t> warm_taken_candidate;
    vector<uint64_t> hot_candidate;
    for (auto &it : btb[cpu][set]) {
      add_to_candidate(it.first, ooo_cpu, cold_candidate,
                       warm_not_taken_candidate, warm_taken_candidate,
                       hot_candidate);
    }
    if (consider_keep) {
      add_to_candidate(ip, ooo_cpu, cold_candidate, warm_not_taken_candidate,
                       warm_taken_candidate, hot_candidate);
    }
    if (!cold_candidate.empty()) {
      ...
      auto taken_history_choice =
          choose_taken_history(cold_candidate, cpu, set);
      if (taken_history_choice != 0)
        return taken_history_choice;
      ...
      auto taken_history_choice =
          choose_taken_history(warm_not_taken_candidate, cpu, set);
      if (taken_history_choice != 0)
        return taken_history_choice;
      ...
      auto taken_history_choice =
          choose_taken_history(warm_taken_candidate, cpu, set);
      if (taken_history_choice != 0)
        return taken_history_choice;
  ...
  //    void add_to_opt_compare_record(uint64_t ip, uint64_t cpu, uint64_t set,
  //    uint64_t hwc_victim, O3_CPU *ooo_cpu) {
  //        auto opt_victim = basic_opt.find_victim(ip, cpu);
  //        evict_compare_record.emplace_back(btb[cpu][set], ip, hwc_victim,
  //        opt_victim, this, ooo_cpu);
  //    }

  void insert(uint64_t ip, uint64_t target, uint8_t branch_type, uint64_t cpu,
              O3_CPU *ooo_cpu) {
    auto set = get_set_index(ip);
    ...
    if (btb[cpu][set].size() >= total_ways) {
      ...
      assert(victim_ip != 0);
      //            coverage_accuracy.get_reuse_distance(victim_ip,
      //            basic_opt.timestamp - 1, victim_ip == ip);
      if (victim_ip == ip)
        return;
  ...
  //            cout << "Evict the " << i << "th category: " <<
  //            general_evict_counter[i]
  //                 << "\t Average candidate: " << (double)
  //                 general_total_counter[i] / (double)
  //                 general_evict_counter[i]
  //                 << endl;
  //        }
  //
  //        // Print compare with opt
  //        cout << "Start print compare with opt with size = " <<
  //        evict_compare_record.size() << endl; string dir =
  //        "/mnt/storage/isaachyw/champsim_pt/hwc_opt_compare_raw_data/hwc_opt_"
  //        + program_name +
  //                     std::to_string(total_btb_ways);
  //        fs::create_directory(dir);
  //        auto short_name = O3_CPU::find_trace_short_name(trace_name,
  //        O3_CPU::NameKind::TRACE_TRAIN); string filename = dir + "/" +
  //        short_name + ".csv"; cout << "Output hwc opt compare to " <<
  //        filename << endl; ofstream out(filename.c_str()); out <<
  //        "0,0,1,1,2,2,3,3,4,4,hwc choice,opt choice" << endl; for (auto &it :
  //        evict_compare_record) {
  //            out << it << endl;
  //        }
  //        out.close();
  //        cout << "End print compare with opt" << endl;
  //        fs::path not_trained_dir =
  //        "/mnt/storage/isaachyw/champsim_pt/input_generalization/not_trained_branches";
  //        fs::create_directory(not_trained_dir);
  //        auto program_not_trained_dir = not_trained_dir / (program_name +
  //        std::to_string(total_btb_ways));
  //        fs::create_directory(program_not_trained_dir);
  //        auto not_trained_file = program_not_trained_dir / (short_name +
  //        ".txt"); cout << "Print not trained branches to " <<
  //        not_trained_file << " with total num: "
...
//         os << (int) entry.candidate_type[i] << "," << (int)
//         entry.candidate_taken[i] << ",";
//     }
...
HotWarmCold hot_warm_cold_btb(BASIC_BTB_SETS, BASIC_BTB_WAYS,
                              BTB_HOT_LOWER_BOUND, BTB_COLD_UPPER_BOUND,
                              BTB_WARM_SPLIT);
...
O3_CPU::btb_prediction(uint64_t ip, uint8_t branch_type, uint64_t *latency) {
  ...
  if (branch_type == BRANCH_RETURN) {
    ...
    if (btb_entry == NULL) {
      ...
      if ((branch_target != 0) && taken) {
        // add the entry to victim buffer inside the insert function
...
void O3_CPU::prefetch_btb(uint64_t ip, uint64_t branch_target,
                          uint8_t branch_type, bool taken,
                          bool to_stream_buffer) {
  // TODO: Here we only prefetch for direct btb, since the hash for indirect
...
void O3_CPU::btb_final_stats() {
  ...
  cout << "BTB taken num: " << predicted_taken_branch_count
       << " BTB miss num: " << btb_miss_taken_branch_count << " BTB miss rate: "
       << ((double)btb_miss_taken_branch_count) /
              ((double)predicted_taken_branch_count)
       << endl;
  //    hot_warm_cold_btb.print_final_stats(trace_name, program_name);
  //   coverage_accuracy.print_final_stats(trace_name, program_name,
  //   BASIC_BTB_WAYS);
