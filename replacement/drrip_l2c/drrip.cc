#include "cache.h"
#include "ctime"
// cache set >= NUM_POLICY * SDM_SIZE = 64
#define maxRRPV 3
#define NUM_POLICY 2
#define SDM_SIZE 32
...
static uint32_t rrpv[L2C_SET][L2C_WAY],
         bip_counter = 0,
         PSEL[NUM_CPUS];
static unsigned rand_sets[TOTAL_SDM_SETS];

void CACHE::l2c_initialize_replacement()
{
    cout << "Initialize DRRIP state on L2C" << endl;
    ...
    for (int i=0; i<TOTAL_SDM_SETS; i++) {
        do {
            ...
            printf("Assign rand_sets[%d]: %u  L2C: %u\n", i, rand_sets[i], my_set);
            for (int j=0; j<i; j++) {
