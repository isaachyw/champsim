...
#define CACHE_SET L1D_SET
#define CACHE_WAY L1D_WAY

#define maxRRPV 3
#define SHCT_SIZE  16384
#define SHCT_PRIME 16381
#define SAMPLER_SET (32*NUM_CPUS)
//cache set >= SAMPLER_SET
#define SAMPLER_WAY CACHE_WAY
#define SHCT_MAX 7

static uint32_t rrpv[CACHE_SET][CACHE_WAY];
...
// sampler
static uint32_t rand_sets[SAMPLER_SET];
static SAMPLER_class sampler[SAMPLER_SET][SAMPLER_WAY];
...
{
    cout << "Initialize SHIP state on LLC" << endl;
    ...
    for (int i=0; i<SAMPLER_SET; i++)
    {
        do 
        {
            ...
            printf("Assign rand_sets[%d]: %u  L1D: %u\n", i, rand_sets[i], my_set);
            for (int j=0; j<i; j++) 
            {
...
static uint32_t is_it_sampled(uint32_t set)
{
    for (int i=0; i<SAMPLER_SET; i++)
        if (rand_sets[i] == set)
            return i;
...
static void update_sampler(uint32_t cpu, uint32_t s_idx, uint64_t address, uint64_t ip, uint8_t type)
{
    ...
    uint64_t tag = address / (64*CACHE_SET); 
    int match = -1;
